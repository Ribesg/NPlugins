...
public class Config extends AbstractConfig {
    ...
    protected void setValues(final YamlConfiguration config) {
        ...
        if (getEnderDragonDamageMultiplier() < 0.0) {
            ...
            plugin.sendMessage(plugin.getServer().getConsoleSender(), MessageId.theEndAgain_incorrectValueInConfiguration, Utils.toLowerCamelCase(worldName) + "Config.yml", "enderDragonDamageMultiplier", "1.0");
        }
        ...
        if (getXpReward() < 0) {
            setXpReward(20_000);
            plugin.sendMessage(plugin.getServer().getConsoleSender(), MessageId.theEndAgain_incorrectValueInConfiguration, Utils.toLowerCamelCase(worldName) + "Config.yml", "xpReward", "20000");
        }
    }
